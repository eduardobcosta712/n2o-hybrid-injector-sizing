...
> **Regenerated in the September 2026 audit.** The previous version of this example quoted 368.1 g/s. That number had been computed with the original, uncorrected Dyer weights (Section 3.4 of the theory docs); with the corrected weights the same case gives 381.8 g/s. All figures below were produced by running the current code.
...
- No flashing detected — pressure at the injector inlet is **57.57 bar**, still well above P_sat(20 °C) = 51.37 bar.
...
| HEM (full equilibrium limit) | 235.2 g/s |
| **Dyer (adopted)** | **381.8 g/s** |
| SPI over-prediction | **25.9%** |

**Dyer model parameters:**
- Non-equilibrium parameter κ = 1.101
- Exit vapour quality x = 0.299 (≈ 30% of the mass exits as vapour)
- Solver converged in 12 iterations (the initial mass-flow guess does not affect the result).

---

## Non-equilibrium choking diagnostic

Running this exact case in the tool also shows a **choking warning**: the Dyer prediction exceeds the Henry-Fauske (1971) non-equilibrium choking ceiling, which at these tank/chamber conditions is **315.3 g/s** — 17% below the 381.8 g/s Dyer prediction.

This does **not** change the 381.8 g/s figure — see `docs/future_work.md`, Priority 1, for why the ceiling is surfaced as a diagnostic warning rather than applied automatically: it is theoretically sound (a primary-source non-equilibrium critical flow model) but has only been confirmed *not* to interfere with the Waxman-validated cases (8–14 bar pressure drop); it has not itself been experimentally validated at this example's conditions (here, roughly 35.6 bar pressure drop, well outside that validated band).

**How to read this result honestly:** the Dyer model itself is validated only at 8–14 bar. At 35 bar the prediction is a model estimate, and the choking diagnostic says the true flow may be as low as ≈315 g/s. A defensible planning range is therefore roughly **315–382 g/s**, to be narrowed by a cold-flow or hot-fire measurement — but in either case far below the SPI figure the motor was designed around.

---

## Interpretation

The SPI model over-predicts the mass flow by 25.9% against the Dyer estimate (and by about 39% if the choking ceiling turns out to be the binding one). If the motor was designed around 515 g/s, the real O/F ratio will be lower than expected, reducing specific impulse and potentially compromising combustion stability.

**Combustion stability check:**
- Injector pressure drop (57.57 − 22 = 35.57 bar) / P_chamber (22 bar) = **162%** — well above the 15% minimum. Stable.
...
5. Read the results — Dyer prediction should show **381.8 g/s** and SPI over-prediction **25.9%**. An amber choking-ceiling warning (315.3 g/s) should also appear underneath the Dyer caption.
