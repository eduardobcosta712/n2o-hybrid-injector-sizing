...
From Section 1.5: $T_{crit}$(N₂O) ≈ 36.4 °C. The practical relevance of this proximity to ambient conditions is tied to the behavior of the **latent heat of vaporization**, $h_{fg}$ — the energy required to convert, at the same pressure/temperature, a unit mass of saturated liquid into saturated vapor.
...
When a fraction of the liquid vaporizes inside the orifice, the mixture's average density, $\rho_{mix}$, drops sharply — the specific volume of saturated N₂O vapor is substantially larger than that of the saturated liquid (potentially, depending on pressure, on the order of tens to hundreds of times larger), so even a small mass-based vapor quality $x$ already occupies most of the available volume in the orifice cross-section.
...
The mixture density follows from the mass-weighted average of the two phases' specific volumes:

$$\rho_{HEM} = \frac{1}{\frac{1-x}{\rho_l} + \frac{x}{\rho_v}}$$
...
where the non-equilibrium parameter

$$\kappa = \sqrt{\frac{P_{up} - P_{down}}{P_{sat}(T_{up}) - P_{down}}}$$

is proportional to $\tau_{bubble}/\tau_{residence}$. The numerator is the total pressure drop; the denominator is how far *below saturation* the downstream pressure lies. $\kappa$ grows with the upstream subcooling and equals 1 for a saturated-liquid inlet ($P_{up} = P_{sat}$). Large $\kappa$ → less equilibrium → more weight on SPI. Small $\kappa$ → near-equilibrium → more weight on HEM. The weight on SPI, $\kappa/(1+\kappa)$, increases with $\kappa$ — consistent with this physical interpretation. $\kappa$ is only defined when the pressure genuinely crosses saturation inside the orifice, i.e. $P_{down} < P_{sat}(T_{up}) < P_{up}$; otherwise SPI (or the two-phase-inlet model below) applies.

> **Implementation note.** The original Dyer et al. (2007) paper contained a sign error in the weighting formula (weights on SPI and HEM were swapped). This was corrected by Solomon (2011) and confirmed by Waxman (2013, Eq. 9). The formula above and the implementation in `injector_two_phase.py` reflect the corrected version. (The worked examples in `examples/` were regenerated in the September 2026 audit; earlier versions of them had been computed with the uncorrected weights.)

Because it reasonably captures observed behaviour in short orifices without the added complexity of more general non-homogeneous, non-equilibrium models, the Dyer model is the most widely adopted in the university rocketry literature for this class of problem, and is the reference model for this project. Validation against Waxman (2013/2014) gives MAPE = 3.51% — **but only at moderate pressure drops of 8–14 bar**; the model has not been validated at the 20–50 bar drops typical of motor designs. See `validation/waxman_2013_results.md`.

### HEM with a two-phase inlet

When the fluid already arrives partially vaporised ($x_{inlet} > 0$, flashing in the feed line), the Dyer blend is not used: its SPI branch represents delayed nucleation in a liquid, and with vapour already present there is no nucleation delay to model, while its inlet state is undefined (the implementation requires a liquid inlet). The HEM limit is applied instead, with upstream enthalpy $h_{up} = h_l(T_{tank}) + x_{inlet}\, h_{fg}(T_{tank})$. Two caveats: this path is implemented and unit tested but **not validated** against experimental data, and the switch from Dyer to HEM at the flashing threshold is **discontinuous** (HEM at $x_{inlet} \to 0$ predicts roughly half the Dyer flow at the same conditions — see `examples/example_03_flashing.md`).
...
---
*Previous document: [02_spi_model.md](02_spi_model.md) · Next document: [03b_grain_sizing.md](03b_grain_sizing.md) — fuel grain sizing; the implementation notes are in [04_implementation.md](04_implementation.md).*
