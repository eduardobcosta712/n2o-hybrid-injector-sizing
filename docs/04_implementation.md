...
**Single-phase region** ($P > P_{\mathrm{sat}}(T_{tank})$): Darcy-Weisbach with pure liquid properties — $\rho_l(T_{tank})$ and $\mu_l(T_{tank})$ from `mu_liquid_sat(T)` (Table A.4, NIST). Previously a constant (`MU_LIQUID_N2O = 1.5\times10^{-4}\,\mathrm{Pa\,s}`); now temperature-dependent.
...
This maximum is the physical choking limit — the two-phase speed-of-sound condition expressed through the isenthalpic path. At Waxman conditions ($T_1 = 280\,\mathrm{K}$, $P_1 = 4.36\,\mathrm{MPa}$): $\dot{m}_{crit} = 41.1\,\mathrm{g/s}$ at $P_{2,crit} = 30.4\,\mathrm{bar}$.
