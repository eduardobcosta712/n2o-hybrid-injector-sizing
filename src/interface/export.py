...
def generate_pdf(mode, inputs, results, segments_ui, model_segments):
    ...
    results : dict
        ...
        Design mode additionally carries "design_regime" ("Dyer" or "SPI",
        default "Dyer"); in the "SPI" regime (flow single-phase through the
        orifice) the model-comparison keys (m_spi, m_hem, m_dyer, ...) are
        absent and the recommended area equals the SPI area.
    segments_ui : list
    ...
    story.append(Paragraph(
        ...
        "fabrication. The Dyer model is validated against experiment only for "
        "injector pressure drops of 8-14 bar. The author accepts no "
        "responsibility for any damages arising from the use of this tool.",
        warn_style))
    ...
    design_regime = results.get("design_regime", "Dyer")

    if mode == "sizing":
        res_data = [["Metric", "Value"]]
        # Feed line
        fl_text = ("YES — two-phase inlet (HEM applied, not validated)"
                   if flashing else "NO")
        ...
        if not flashing:
            ...
            if design_regime == "Dyer":
                ...
                res_data.append(["Recommended total area (SPI valid)",
                                  f"{results.get('A_dyer', 0)*1e6:.4f} mm²"])
                res_data.append(["Note", "Single-phase through the orifice — "
                                          "no Dyer correction needed"])
